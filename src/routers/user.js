...
const upload = multer({
    ...
    fileFilter(req, file, cb) {
        if (!/.(jpg|jpeg|png|svg)$/.test(file.originalname)) {
...
router.post("/users/login", async (req, res) => {
    try {
        ...
        //call res.send will call JSON.stringify on user behind the scene
        res.send(user);
    } catch (e) {
        res.status(400).send();
    }
...
//should not support this request to see all users' info
